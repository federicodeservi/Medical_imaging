...
clear;
root = 'S:\Uni\2 anno\Big Data Medical Imaging\Progetto\manifest-1618047023244\Lung-PET-CT-Dx';
...
for n = 1:n_imgs
    ...
    info = dicominfo(fullfile(path,files(n*5).name));
...
elseif isempty(frameNum)==1
    ...
    S2.f = figure;
    close(S2.f);
    info = dicominfo(fullfile(path,files(frameNum).name));
    slope = info.RescaleSlope;
    intercept = info.RescaleIntercept;
